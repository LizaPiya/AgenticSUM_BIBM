"""
AgenticSum with Mistral-7B-Instruct-v0.3 backbone.

Identical to run_agenticsum.py and Soap_run_agenticsum.py in every way EXCEPT
the backbone model name. All agent parameters, prompts, evaluation metrics,
and judge setup are unchanged.

Runs MIMIC and SOAP sequentially, then computes evaluation + LLM-as-judge.
"""
...
# ======================================================
# Model — only this line differs from run_agenticsum.py
# ======================================================
...
def run_pipeline(dataset_name, data_path):
    ...
    print(f"\n{'='*80}")
    print(f"Starting AgenticSum [{dataset_name.upper()}] — {len(df)} samples")
    print(f"{'='*80}\n")
    ...
    with torch.no_grad():
        for batch_start in range(0, len(df), BATCH_SIZE):
            ...
            for idx, row in batch_df.iterrows():
                if len(str(row["input"])) > 30000:
                    print(f"[{idx+1}] Skipping {row['note_id']}: too long")
                    results.append({
                        "note_id": row["note_id"],
                        "input": "",
                        "target": row["target"],
    ...
    out_path = f"{OUTPUT_DIR}/mistral7b_{dataset_name}_summaries.csv"
    results_df.to_csv(out_path, index=False)

    success = len([r for r in results if "ERROR" not in str(r.get("fixed_summary","")) and "SKIPPED" not in str(r.get("fixed_summary",""))])
...
for name in DATASETS:
    summary_path = f"{OUTPUT_DIR}/mistral7b_{name}_summaries.csv"
...
# ======================================================
# Step 3 — LLM-as-judge with Meta-Llama-3-8B-Instruct
#          Same model and same function as Table 2
# ======================================================
print("\n" + "="*80)
print("Loading judge: meta-llama/Meta-Llama-3-8B-Instruct (float16)")
print("="*80)

del model, tokenizer
gc.collect()
torch.cuda.empty_cache()
print(f"GPU mem after freeing Mistral: {torch.cuda.memory_allocated()/1e9:.1f} GB")
...
for name in DATASETS:
    summary_path = f"{OUTPUT_DIR}/mistral7b_{name}_summaries.csv"
    out_path     = f"{OUTPUT_DIR}/mistral7b_{name}_results.csv"
    ...
    print(f"\n{'='*60}")
    print(f"FINAL RESULTS — AgenticSum-Mistral7B — {name.upper()}")
    print(f"{'='*60}")
    for label, col in [
        ("ROUGE-L",         "rouge_l"),
        ("BLEU-1",          "bleu1"),
