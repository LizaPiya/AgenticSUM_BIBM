"""
run_ablation_soap.py
--------------------
Additive ablation study for AgenticSum — SOAP (100 documents).
...
print(f"Running ablation on {len(df)} SOAP documents\n")

# ── Output ────────────────────────────────────────────────────────────────────
output_dir = "/home/user/MLHC_AgenticSUM/outputs/agenticsum"
os.makedirs(output_dir, exist_ok=True)
os.makedirs(f"{output_dir}/checkpoints_ablation_soap", exist_ok=True)

results = []
BATCH_SIZE = 5

print("=" * 80)
print("ABLATION STUDY — SOAP (100 documents, 4 conditions)")
print("=" * 80 + "\n")
...
with torch.no_grad():
    for batch_start in range(0, len(df), BATCH_SIZE):
        ...
        for idx, row in batch_df.iterrows():
            ...
            # C3: +FOCUS +Detect +Fix (single pass)
            try:
        ...
        checkpoint_path = f"{output_dir}/checkpoints_ablation_soap/batch_{batch_start//BATCH_SIZE + 1}.csv"
        pd.DataFrame(results).to_csv(checkpoint_path, index=False)
        print(f"\n💾 Checkpoint saved: {len(results)}/100\n")
        torch.cuda.empty_cache(); gc.collect()

results_df = pd.DataFrame(results)
results_df.to_csv(f"{output_dir}/ablation_results_soap.csv", index=False)
print(f"\nResults saved -> {output_dir}/ablation_results_soap.csv")
print("=" * 80)
