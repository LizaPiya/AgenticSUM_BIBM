"""
run_aura_ablation_soap.py
-------------------------
AURA-component ablation — SOAP (100 documents).
...
# ── Three detectors ───────────────────────────────────────────────────────────
detector_a1 = HallucinationDetectorAgent(
    model=model, tokenizer=tokenizer,
    semantic_judge=real_semantic_judge, aura_threshold=0.0,
)
detector_a2 = HallucinationDetectorAgent(
    model=model, tokenizer=tokenizer,
    semantic_judge=mock_semantic_judge, aura_threshold=0.42,
)
detector_a3 = HallucinationDetectorAgent(
    model=model, tokenizer=tokenizer,
...
# ── Data ──────────────────────────────────────────────────────────────────────
df = pd.read_csv("../Dataset/df_soap_mimic.csv").head(100).reset_index(drop=True)
...
print("=" * 80)
print("AURA ABLATION — SOAP (100 documents, 3 conditions)")
print("=" * 80 + "\n")
...
with torch.no_grad():
    for batch_start in range(0, len(df), BATCH_SIZE):
        ...
        checkpoint_path = f"{output_dir}/checkpoints_aura_soap/batch_{batch_start//BATCH_SIZE + 1}.csv"
        pd.DataFrame(results).to_csv(checkpoint_path, index=False)
        print(f"\n💾 Checkpoint saved: {len(results)}/100\n")
        torch.cuda.empty_cache(); gc.collect()

results_df = pd.DataFrame(results)
results_df.to_csv(f"{output_dir}/aura_ablation_100docs_SOAP.csv", index=False)
print(f"\nResults saved -> {output_dir}/aura_ablation_100docs_SOAP.csv")
print("=" * 80)
